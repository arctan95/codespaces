...
The container includes the `zsh` (and Oh My Zsh!) shell that you can opt into using instead of the default `bash`. It also includes [Nix](https://nixos.org/) as package manager. You can also set things up to access the container via SSH.
...
If you don't want to use docker image, you can alternatively run the command below. Notice that you must install Nix first and set your username to `codespace`.

`nix --extra-experimental-features "nix-command flakes" run nixpkgs#home-manager -- switch --flake github:arctan95/codespaces`
